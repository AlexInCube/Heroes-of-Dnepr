function calculaterandom( keys )
	...
	local slow_max = ability:GetLevelSpecialValueFor("slowduration_max", ability_level)
	local slow_min = ability:GetLevelSpecialValueFor("slowduration_min", ability_level)
	...
	local slowduration = slow_min + (slow_max - slow_min) * (1 - random)
	...
	target:AddNewModifier(caster, ability, "modifier_fireblast_datadriven", {duration = stun})
	target:AddNewModifier(caster, ability, "modifier_xoxolslow_datadriven", {duration = slowduration})

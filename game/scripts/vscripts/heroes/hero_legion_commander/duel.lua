function legion_commander_duel_datadriven_on_spell_start(keys)
	...
	caster.legion_commander_duel_datadriven_particle = ParticleManager:CreateParticle("particles/customgames/capturepoints/cp_mover.vpcf", PATTACH_ABSORIGIN, caster)
...
function modifier_duel_datadriven_on_death(keys)
	...
	if unit == caster then  --If Legion Commander was killed.
		...
		local duel_victory_particle = ParticleManager:CreateParticle("particles/dire_fx/bad_ancient002_pit_lava_blast_lava.vpcf", PATTACH_ABSORIGIN_FOLLOW, caster)

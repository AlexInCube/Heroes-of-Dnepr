function invoker_chaos_meteor_datadriven_on_spell_start(keys)
	...
	local exort_ability = keys.caster:FindAbilityByName("chaos_meteor_datadriven")
	local main_damage = 0
	local burn_dps = 0
	if exort_ability ~= nil then
		...
		burn_dps = keys.ability:GetLevelSpecialValueFor("burn_dps", exort_level - 1)/100
	end
	...
	local wex_ability = keys.caster:FindAbilityByName("chaos_meteor_datadriven")
...
function modifier_invoker_chaos_meteor_datadriven_burn_damage_on_interval_think(keys)
	...
	local health = target:GetMaxHealth()
		ApplyDamage({victim = keys.target, attacker = keys.caster.invoker_chaos_meteor_parent_caster, damage = (health * keys.caster.invoker_chaos_meteor_burn_dps) * keys.BurnDPSInterval, damage_type = DAMAGE_TYPE_MAGICAL,})
	end
end
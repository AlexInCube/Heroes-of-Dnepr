...
function modifier_item_abyssal_blade_datadriven_bash_chance_on_attack_landed(keys)
	if not keys.caster:HasModifier("bash_cooldown_modifier") then
		...
		if (is_ranged_attacker and random_int <= keys.BashChanceRanged) or (not is_ranged_attacker and random_int <= keys.BashChanceMelee) then
			...
			keys.ability:ApplyDataDrivenModifier(keys.caster, keys.target, "modifier_item_banhammer_datadriven_bash", nil)
...
function item_abyssal_blade_datadriven_on_spell_start(keys)
	...
	keys.ability:ApplyDataDrivenModifier(keys.caster, keys.target, "modifier_item_banhammer_datadriven_active", nil)
...
function modifier_item_abyssal_blade_datadriven_recalculate_bash_chance(keys)
	Timers:CreateTimer({
		callback = function()
			...
			while keys.caster:HasModifier("modifier_item_banhammer_bash_chance") do
				keys.caster:RemoveModifierByName("modifier_item_banhammer_datadriven_bash_chance")
			end
			...
			for i=0, 5, 1 do
				...
				if current_item ~= nil then
					...
					if item_name == "item_abyssal_blade_datadriven" then
						skull_basher = current_item
					elseif item_name == "item_banhammer" then
			...
			if abyssal_blade ~= nil then  --Prioritize the Abyssal Blade bash chance modifier.
				abyssal_blade:ApplyDataDrivenModifier(keys.caster, keys.caster, "modifier_item_banhammer_datadriven_bash_chance", {duration = -1})
			elseif skull_basher ~= nil then
				skull_basher:ApplyDataDrivenModifier(keys.caster, keys.caster, "modifier_item_abyssal_blade_datadriven_bash_chance", {duration = -1})
			end
		end
	})
end